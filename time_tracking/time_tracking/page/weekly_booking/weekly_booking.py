...
def _get_increment_minutes():
	return cint(frappe.db.get_single_value("Time Tracking Settings", "time_booking_increment_minutes") or 15)
...
def _get_assigned_projects(user, profile_name=None):
	profile_name = profile_name or _require_profile(user)
...
def get_weekly_booking(user=None, week_start_date=None):
	...
	calendar_week, calendar_year, week_end_date, period_label, warning = _get_week_info(week_start_date)
...
def get_assigned_projects(user=None):
	...
	return _get_assigned_projects(user, profile_name=profile_name)
...
def save_weekly_booking(data):
	...
	if existing_minutes and not_bookable_projects:
		existing_not_bookable_minutes = {
			key: minutes for key, minutes in existing_minutes.items() if key[0] in not_bookable_projects
		}
		...
		if existing_minutes:
			if assigned_project_names:
				existing_unassigned_minutes = {
					...
					if key[0] not in assigned_project_names and key[0] not in not_bookable_projects
				}
			else:
				existing_unassigned_minutes = {
					key: minutes
	...
	try:
		...
		if existing_not_bookable_minutes or row_not_bookable_minutes:
			...
			if mismatched_projects:
				...
				frappe.throw(_("Projects not bookable cannot be edited: {0}").format(project_list))
