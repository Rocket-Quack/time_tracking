...
def send_booking_reminders():
	...
	if trigger == TRIGGER_MONTHLY and not _is_reminder_day(settings.monthly_booking_reminder_day, today):
		return
	if trigger == TRIGGER_MONTH_END and not _is_month_end(today):
		return
	if trigger == TRIGGER_WEEKLY and not _is_weekday_match(settings.booking_reminder_weekday, today):
		return
	...
	for profile in _get_profiles():
		...
		if trigger == TRIGGER_INACTIVITY:
			...
			if not missing_dates:
				continue
			message = _("You have not booked time for {0} days. Please update your time bookings.").format(
				inactive_days
			)
