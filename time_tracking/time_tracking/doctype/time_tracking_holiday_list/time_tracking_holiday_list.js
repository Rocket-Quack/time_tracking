...
function open_fetch_dialog(frm) {
	const dialog = new frappe.ui.Dialog({
		...
		primary_action(values) {
			...
			frappe.call({
				method: "time_tracking.time_tracking.doctype.time_tracking_holiday_list.time_tracking_holiday_list.fetch_open_holidays",
				args: {
					country_code,
					year: selected_year,
				},
				...
				callback: (response) => {
					...
					frappe.show_alert({
						message: __("Fetched {0} holidays. Please save to apply changes.").format(
							holidays.length
						),
						indicator: "green",
					});
...
function load_holiday_countries(dialog, continent) {
	...
	if (!open_holiday_country_request) {
		open_holiday_country_request = frappe.call({
			method: "time_tracking.time_tracking.doctype.time_tracking_holiday_list.time_tracking_holiday_list.get_open_holiday_countries",
			freeze: true,
			freeze_message: __("Fetching countries..."),
		});
	}

	open_holiday_country_request
		.then((response) => {
