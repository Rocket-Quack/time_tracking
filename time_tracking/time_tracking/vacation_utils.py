...
def get_holidays_enabled():
	return cint(frappe.db.get_single_value("Time Tracking Settings", "enable_holiday_list") or 0)


def get_allow_negative_vacation_balance():
	return cint(frappe.db.get_single_value("Time Tracking Settings", "allow_negative_vacation_balance") or 0)


def get_default_workdays_per_week():
	value = flt(frappe.db.get_single_value("Time Tracking Settings", "default_workdays_per_week") or 0)
	return value if value > 0 else 5


def _get_vacation_year_end_policy():
	return frappe.db.get_single_value("Time Tracking Settings", "vacation_year_end_policy") or "Cap"


def _get_vacation_carryover_limit():
	return flt(frappe.db.get_single_value("Time Tracking Settings", "vacation_carryover_limit_days") or 0)
...
def _compute_vacation_carryover_days(profile, year):
	...
	used_minutes = get_vacation_used_minutes(profile.name, getdate(f"{previous_year}-12-31"))
...
def _upsert_vacation_ledger_entry(values):
	...
	existing_name = frappe.db.get_value("Time Tracking Vacation Ledger", filters, "name")
	if existing_name:
		frappe.db.set_value(
			"Time Tracking Vacation Ledger",
			existing_name,
...
def get_vacation_balance(profile, date, exclude_booking_name=None):
	...
	allowance_minutes = (
		(flt(profile.vacation_days_per_year) + flt(carryover_days) + flt(opening_days)) * hours_per_day * 60
	)
	used_minutes = get_vacation_used_minutes(profile.name, date, exclude_booking_name=exclude_booking_name)
...
def handle_vacation_booking_change(doc, method=None):
	...
	for year in sorted(years):
		carryover_days = _compute_vacation_carryover_days(profile, year)
		_sync_vacation_year_end_carryover(profile, year, carryover_days, source="time_booking")
...
def recalculate_vacation_ledger(profile_name=None, start_year=None, end_year=None):
	...
	for profile in profiles:
		...
		for year in range(start_year, end_year + 1):
			...
			_sync_vacation_year_end_carryover(profile, year, carryover_days, source="recalc_job")
