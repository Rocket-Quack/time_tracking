frappe.pages["weekly-booking"].on_page_load = function (wrapper) {
	...
	const styles = `
        <style>
            body[data-route="weekly-booking"] {
                ...
                --tt-wb-header-input-bg: var(--card-bg, var(--fg-color, #ffffff));
                --tt-wb-header-input-border: var(--border-color, #d1d8dd);
                --tt-wb-header-input-text: var(--text-color, #1f272e);
                --tt-wb-header-input-focus: rgba(47, 128, 237, 0.2);
            ...
            [data-theme="dark"] .weekly-booking-page {
                ...
                --tt-wb-header-input-text: #dde7f1;
                --tt-wb-header-input-focus: rgba(91, 163, 246, 0.3);
            ...
            .weekly-booking #weekly-booking-header .form-group > label {
                color: var(--tt-wb-muted);
            }
            .weekly-booking #weekly-booking-user,
            .weekly-booking #weekly-booking-week-start,
            .weekly-booking #weekly-booking-calendar-week {
                background: var(--tt-wb-header-input-bg);
            ...
            .weekly-booking #weekly-booking-user.form-control[readonly],
            .weekly-booking #weekly-booking-week-start.form-control[readonly],
            .weekly-booking #weekly-booking-calendar-week.form-control[readonly] {
                -webkit-text-fill-color: var(--tt-wb-header-input-text);
                opacity: 1;
            }
            .weekly-booking #weekly-booking-user:focus,
            .weekly-booking #weekly-booking-week-start:focus,
            .weekly-booking #weekly-booking-calendar-week:focus {
                border-color: var(--tt-wb-header-input-border);
                box-shadow: 0 0 0 2px var(--tt-wb-header-input-focus);
            }
            ...
            @media (max-width: 1199px) {
                .weekly-booking .weekly-booking-table thead .wb-actions-col {
                    position: sticky;
                    right: 0;
                    z-index: 6;
                    background: var(--tt-wb-sticky-col-bg);
                    box-shadow: var(--tt-wb-sticky-col-shadow);
                }
                .weekly-booking .weekly-booking-table tbody .wb-actions-cell {
                    position: sticky;
                    right: 0;
                    z-index: 5;
                    background: var(--tt-wb-sticky-col-bg);
                ...
                .weekly-booking .weekly-booking-table tfoot td:last-child {
                    position: sticky;
                    right: 0;
                    z-index: 4;
	...
	function handleProfileMissingError(error) {
		const message = getServerErrorMessage(error);
		if (!message) {
			return false;
		}
		if (message === profileMissingMessage || message.includes(profileMissingMessage)) {
			handleProfileMissing(message);
			return true;
		}
		return false;
	}

	function handleProfileMissing(message) {
		if (state.profile_missing_handled) {
			return;
		}
		state.profile_missing_handled = true;
		$addRowButton.prop("disabled", true);
		if (page && page.btn_primary) {
			page.btn_primary.prop("disabled", true);
		}

		const isAdmin = isTimeTrackingAdmin();
		const text = message || profileMissingMessage;
		const dialog = new frappe.ui.Dialog({
			title: profileMissingTitle,
			fields: [
				{
					fieldtype: "HTML",
	...
	function scrollToDayColumn(dayIndex) {
		...
		const headerLeft = $header.offset().left - $wrap.offset().left + $wrap.scrollLeft();
		const padding = 12;
		...
		frappe.call({
			...
			callback: function (r) {
				...
				if (message.profile_missing) {
					handleProfileMissing(message.message);
					return;
				}
		...
		$table.find(".wb-day-header").each(function () {
			...
			if (holidaySet.has(dateStr)) {
				...
				const holidayLabel = holidayMinutes
					...
					: __("Feiertag");
				$header.prepend(`<div class="wb-day-holiday-label">${holidayLabel}</div>`);
				state.holiday_minutes_by_day[offset] = Math.round(
					state.holiday_hours_per_day * 60
				);
				$table.find(`thead th:nth-child(${columnIndex})`).addClass("wb-day-holiday-col");
				...
				$table.find(`tfoot td:nth-child(${columnIndex})`).addClass("wb-day-holiday-col");
			} else {
