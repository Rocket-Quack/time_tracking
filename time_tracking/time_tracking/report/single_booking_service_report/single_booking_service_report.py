...
from frappe.utils import add_days, flt, getdate, nowdate, strip_html
...
def execute(filters=None):
	...
	project_names = {row.project for row in rows if row.project}
	path_labels = build_project_path_labels(list(project_names))
		...
		data.append(
			{
				"date": row.date,
				"user": row.user,
				"employee_name": row.employee_name or row.user,
				"project": row.project,
				"project_label": path_labels.get(row.project, row.project or ""),
				"note": strip_html(row.note or ""),
