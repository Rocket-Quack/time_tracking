...
function buildConversionPreviewHtml(baseValue, weeksPerMonth, suggestedValue, targetPeriod) {
	...
	const baseLabel = targetPeriod === "Monthly" ? __("Weekly Target") : __("Monthly Target");
...
function openTargetSwitchDialog(frm, newPeriod) {
	...
	const baseValue =
		newPeriod === "Monthly" ? frm.doc.weekly_target_hours : frm.doc.monthly_target_hours;
	...
	getTrackTargetAdjustmentsSetting(frm).then((trackAdjustments) => {
		...
		const dialog = new frappe.ui.Dialog({
			...
			primary_action(values) {
				...
				frappe.call({
					method: "time_tracking.time_tracking.doctype.time_tracking_profile.time_tracking_profile.switch_target_period",
					args: {
		...
		function updateSuggestion() {
			...
			const suggested =
				newPeriod === "Monthly" ? Number(baseValue) * weeks : Number(baseValue) / weeks;
	...
	getTrackTargetAdjustmentsSetting(frm).then((trackAdjustments) => {
		...
		frappe.prompt(
			...
			(values) => {
				...
				frappe.call({
					method: "time_tracking.time_tracking.doctype.time_tracking_profile.time_tracking_profile.adjust_target",
					args: {
						profile_name: frm.doc.name,
...
function addTargetButtons(frm) {
	...
	const labelSuffix = targetPeriod === "Weekly" ? __("Weekly Target") : __("Monthly Target");
