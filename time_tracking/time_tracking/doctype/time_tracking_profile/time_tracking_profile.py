	...
	def _validate_project_assignment_permissions(self):
		...
		if previous:
			for row in previous.project_assignments or []:
				if not row.project:
					continue
				previous_map[row.project] = int(row.active) if row.active is not None else 0

		newly_active = {
			project for project, active in current_map.items() if active and previous_map.get(project, 0) == 0
		}

		if not newly_active:
			return

		expanded_active = expand_project_assignments(list(newly_active), include_not_bookable=True)
		if not expanded_active:
			return

		projects = frappe.get_all(
			"Time Tracking Project",
			filters={"name": ["in", list(expanded_active)]},
			fields=["name", "assignment_mode"],
		)
		restricted = {
			project.name for project in projects if (project.assignment_mode or "Open") == "Restricted"
		}
		if not restricted:
			return
		...
		if unauthorized:
			...
			frappe.throw(
				_("You are not allowed to self-assign the following projects: {0}").format(project_list)
			)
...
def _track_target_adjustments_enabled():
	return cint(frappe.db.get_single_value("Time Tracking Settings", "track_target_adjustments") or 0)
