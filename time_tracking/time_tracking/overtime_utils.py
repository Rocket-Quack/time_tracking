...
def _get_booking_minutes_by_date(profile_name, start_date, end_date, include_vacation, include_sickness):
	rows = frappe.get_all(
		"Time Booking",
		filters={
			"time_tracking_profile": profile_name,
	...
	for row in rows:
		...
		minutes_by_date[date_key] = minutes_by_date.get(date_key, 0) + int(flt(row.duration_minutes))
...
def calculate_period_overtime(profile, start_date, end_date, period_type, settings):
	totals_by_project, total_minutes = _get_time_booking_totals(profile.name, start_date, end_date)
...
def _upsert_ledger_entry(values):
	...
	existing_name = frappe.db.get_value("Time Tracking Overtime Ledger", filters, "name")
	if existing_name:
		frappe.db.set_value(
			"Time Tracking Overtime Ledger",
			existing_name,
...
def _recalculate_overtime_periods(profile, start_date, end_date, settings):
	...
	while current <= end_date:
		...
		totals_by_project, total_minutes = _get_time_booking_totals(profile.name, period_start, period_end)
...
def recalculate_overtime_ledger(profile_name=None, start_date=None, end_date=None):
	...
	for profile in profiles:
		...
		updated, deleted = _recalculate_overtime_periods(profile, range_start, range_end, settings)
