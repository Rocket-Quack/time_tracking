...
def execute(filters=None):
	...
	overtime_carryover_minutes = get_overtime_carryover_minutes(profile, year, settings=settings)
	...
	for month in range(1, 13):
		...
		totals_by_project, total_minutes = _get_booking_totals(profile.name, start_date, end_date)
		...
		rows.append(
			{
				...
				"overtime_carryover": (flt(overtime_carryover_minutes / 60, 2) if month == 1 else None),
				...
				"monthly_delta": (flt(delta_minutes / 60, 2) if has_bookings else None),
				"overtime_balance": (flt(running_minutes / 60, 2) if has_bookings else None),
				"vacation_carryover": (flt(vacation_carryover_days, 2) if month == 1 else None),
